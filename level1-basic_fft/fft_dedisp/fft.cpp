...
void fft_dedisp(int DM, DTYPE X_R[SIZE], DTYPE X_I[SIZE], DTYPE FFT_R[SIZE], DTYPE FFT_I[SIZE], DTYPE OUT_R[SIZE], DTYPE OUT_I[SIZE]) {
	DTYPE FFT_TEMP_R[SIZE];
	DTYPE FFT_TEMP_I[SIZE];
	fft(X_R, X_I);
	for (int i = 0; i < SIZE; i++)
	{
		FFT_R[i] = X_R[i];
	...
	co_dedisp(40, FFT_TEMP_R, FFT_TEMP_I, OUT_R, OUT_I);
}
...
void fft_dedisp(int DM, DTYPE X_R[SIZE], DTYPE X_I[SIZE], DTYPE FFT_R[SIZE], DTYPE FFT_I[SIZE], DTYPE OUT_R[SIZE], DTYPE OUT_I[SIZE]) {
	DTYPE FFT_TEMP_R[SIZE];
	DTYPE FFT_TEMP_I[SIZE];
	fft(X_R, X_I, FFT_TEMP_R, FFT_TEMP_I);
	for (int i = 0; i < SIZE; i++)
	{
		FFT_R[i] = FFT_TEMP_R[i];
		FFT_I[i] = FFT_TEMP_I[i];
	}
